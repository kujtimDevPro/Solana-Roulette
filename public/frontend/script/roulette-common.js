...
function fillRoulette(cases) {
  ...
  arr.forEach(function (item, i, arr) {
    ...
    el += '<div class="itm ' + item[0] + '" style="height:85px; width:85px"></div>'
    //  +
...
function amountMul(amountVal) {
  ...
  if (betAmt) {
    ...
    (amountVal == 'Clear') ? $('#betAmounts').val('0') : '';

    (amountVal == 'pointZeroFive') ? $('#betAmounts').val((betAmt + 0.05).toFixed(2)) : '';
    (amountVal == 'pointOne') ? $('#betAmounts').val((betAmt + 0.1).toFixed(2)) : '';
    (amountVal == 'pointTwoFive') ? $('#betAmounts').val((betAmt + 0.25).toFixed(2)) : '';
    ...
    (amountVal == 'pointZeroFive') ? $('#betAmounts').val((0.05).toFixed(2)) : '';
    (amountVal == 'pointOne') ? $('#betAmounts').val((0.1).toFixed(2)) : '';
    (amountVal == 'pointTwoFive') ? $('#betAmounts').val((0.25).toFixed(2)) : '';
  ...
  if (!userId) {
    ...
    socket.emit('bettingStart', { 'user_id': userId, 'bet_amount': betAmt, 'btn_click': btn_click }, function (response) {
      if (response.status == "success") {
        ...
        // $(".three_tbl").css({ "opacity": "0.4", "pointer-events": "none" })
        if(btn_click == "danger") $('.black_bg_btn').attr('disabled','disable');
        if(btn_click == "grey") $('.danger_bg_btn').attr('disabled', 'disable')
      } else {
        $.toast({ heading: 'Error', text: response.message, position: 'top-right', icon: 'error', stack: false });
      }
    });
...
socket.on("rouletteStoppedUpdate", function (response) {
  ...
  for (var i = 0; i < response.length; i++) {
    if (response[i].game_stopped_on == "danger") {
      html += '<li class="badges danger-gradiant" style="margin-left:5px"></li>'
    } else if (response[i].game_stopped_on == "green") {
      html += '<li class="badges success-gradiant" style="margin-left:5px"></li>'
    } else if (response[i].game_stopped_on == "black") {
      html += '<li class="badges dark-gradiant" style="margin-left:5px"></li>'
    }
  }
...
socket.on("rouletteClearBetPlayer", function (response) {
  ...
  $('.black_bg_btn').removeAttr('disabled');
  $('.danger_bg_btn').removeAttr('disabled');
  fillRoulette(cases);
  // $("#minutes").text(10)
});
//Start: On Roulette Game start remove all bet player
...
socket.on("showNotification", function (response) {
  ...
  for (let i = 0; i < response.length; i++) {
    if (response[i].user_id == userIds && response[i].is_won == 'yes') {
      ...
      $.toast({ heading: 'Oops!', text: 'You lost the bet. ', position: 'top-right', icon: 'error', stack: false });
    }
  }
});
//Start: Show notification
